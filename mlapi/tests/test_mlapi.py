...
# Bad Test Case: sending "bad" data by incorrect key
def test_predict_bad_request(client):
    data = {"wrong_key": ["This should fail."]}
    response = client.post(
        "/project/bulk-predict",
        json=data,
    )
    print(response.json())  # Print the response for debugging
    assert response.status_code == 422 # The response to have a 422 status code due to bad input data